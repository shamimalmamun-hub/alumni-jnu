...
interface MemberCardModalProps {
  ...
  onOpenFormPdf?: (member: CardMemberData) => void;
}
...
export const MemberCardModal: React.FC<MemberCardModalProps> = ({
  ...
  const isLifeMember =
    ...
    member.feeAmount === '2500' ||
    member.feeAmount === '২৫০০' ||
    (member as any).purposeType === 'membership_life' ||
    (member as any).feePurpose === 'membership_life' ||
    (member as any).membershipCategory === 'life';

  const memberTypeTitle = isLifeMember ? 'আজীবন সদস্য' : 'সাধারণ সদস্য';
          ...
          try {
            ...
            if (navigator.canShare({ files: [file] })) {
              await navigator.share({
                ...
                text: `উদ্ভিদবিজ্ঞান অ্যালামনাই অ্যাসোসিয়েশন - ${memberTypeTitle} (${displayFormNo})`,
              });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-200 app-modal-overlay">
      <div className="bg-white max-w-sm sm:max-w-md w-full rounded-2xl shadow-2xl border-2 border-emerald-700/50 overflow-hidden p-3.5 sm:p-5 space-y-3.5 relative my-auto">
          ...
          <div className="text-center border-b border-emerald-700/20 pb-2.5 space-y-1.5">
            ...
            <div className="pt-0.5">
              <div
                className={`${
                  isLifeMember
                    ? 'bg-gradient-to-r from-amber-500 via-amber-600 to-amber-500 text-amber-950 border border-amber-300'
                    : 'bg-[#006a4e] text-white'
                } px-3 py-1 rounded-md shadow-xs inline-block`}
              >
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 no-print">
          ...
          {onOpenFormPdf && (
            <button
              type="button"
              onClick={() => onOpenFormPdf(member)}
              className="min-h-[44px] py-2.5 px-3.5 bg-emerald-50 hover:bg-emerald-100 text-[#006a4e] border border-emerald-300 font-bold text-xs sm:text-sm rounded-xl transition-colors cursor-pointer flex items-center gap-1.5 shadow-2xs"
              title={language === 'bn' ? 'পূরণকৃত আবেদন ফরমের PDF দেখুন' : 'View Filled Form PDF'}
            >
              <FileText className="w-4 h-4" />
              <span>{language === 'bn' ? 'আবেদন ফরম (PDF)' : 'Form PDF'}</span>
            </button>
          )}
