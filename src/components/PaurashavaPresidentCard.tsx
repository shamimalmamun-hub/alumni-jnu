...
import { BookOpen, MessageSquare, Award, Library } from 'lucide-react';
import { PAURASHAVA_PRESIDENT_BIO } from '../data/portalData';

const SEMINAR_INCHARGE_PHOTO = 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=400&q=80';

interface PaurashavaPresidentCardProps {
  language: 'bn' | 'en';
  onOpenSpeechModal: () => void;
}

export const PaurashavaPresidentCard: React.FC<PaurashavaPresidentCardProps> = ({
  language,
  onOpenSpeechModal,
}) => {
  return (
    <div id="seminar-incharge-card" className="bg-white border-2 border-emerald-700/30 rounded-xl shadow-md overflow-hidden flex flex-col font-siliguri h-full transition-all hover:shadow-lg">
      {/* Green Header Bar */}
      <div className="bg-gradient-to-r from-[#064e3b] via-[#047857] to-[#059669] text-white px-2.5 sm:px-3 py-2.5 flex items-center justify-between border-b-2 border-amber-400 min-w-0">
        <div className="flex items-center gap-1.5 font-bold tracking-tight min-w-0 w-full overflow-hidden">
          <Library className="w-4 h-4 sm:w-5 sm:h-5 text-amber-300 shrink-0" />
          <h2 className="text-[10px] min-[360px]:text-[11px] sm:text-xs md:text-sm font-bold whitespace-nowrap leading-none tracking-normal">
            {language === 'bn' ? 'সেমিনার ও লাইব্রেরি ইনচার্জ' : 'Seminar & Library In-Charge'}
          </h2>
        </div>
      ...
      <div className="p-3.5 flex flex-col space-y-3 bg-gradient-to-b from-[#f0fdf4] via-white to-[#f8fafc] flex-1 justify-between">
        <div className="space-y-3">
          ...
          <div className="flex gap-3 items-center">
            ...
            <div className="w-22 sm:w-26 h-30 sm:h-34 rounded-xl overflow-hidden shadow-md border-2 border-[#047857] ring-2 ring-amber-400/60 bg-gray-100 shrink-0">
              <img
                src={SEMINAR_INCHARGE_PHOTO}
                alt={language === 'bn' ? PAURASHAVA_PRESIDENT_BIO.nameBn : PAURASHAVA_PRESIDENT_BIO.nameEn}
                className="w-full h-full object-cover object-top"
                onError={(e) => {
                  e.currentTarget.onerror = null;
                  e.currentTarget.src = SEMINAR_INCHARGE_PHOTO;
                }}
              />
            </div>
            ...
            <div className="flex-1 min-w-0 space-y-1 text-left">
              ...
              <h3 className="text-sm sm:text-base font-bold text-[#0f172a] leading-tight pt-0.5">
                {language === 'bn' ? PAURASHAVA_PRESIDENT_BIO.nameBn : PAURASHAVA_PRESIDENT_BIO.nameEn}
              </h3>
              <p className="text-xs font-bold text-amber-700">
                {language === 'bn' ? PAURASHAVA_PRESIDENT_BIO.titleBn : PAURASHAVA_PRESIDENT_BIO.titleEn}
              </p>
              <p className="text-[11px] text-slate-600 font-medium leading-tight">
                {language === 'bn' ? PAURASHAVA_PRESIDENT_BIO.ministryBn : PAURASHAVA_PRESIDENT_BIO.ministryEn}
              </p>
            </div>
          </div>
          ...
          <div className="bg-[#ecfdf5] border-l-4 border-[#047857] p-2.5 rounded-r-lg text-left shadow-2xs">
            <p className="text-xs text-[#065f46] font-medium leading-relaxed italic">
              "{language === 'bn' ? PAURASHAVA_PRESIDENT_BIO.messageBn : PAURASHAVA_PRESIDENT_BIO.messageEn}"
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          <button
            id="btn-seminar-incharge-speech"
