...
import { MessageSquare, BookOpen, Leaf, Wallet, Phone, Mail, GraduationCap } from 'lucide-react';
import { TREASURER_BIO } from '../data/portalData';

const TREASURER_PHOTO_URL = '/tresure.jpeg';
...
export const TreasurerCard: React.FC<TreasurerCardProps> = ({
  language,
  onOpenSpeechModal,
}) => {
  return (
    <div id="treasurer-card" className="bg-white border border-[#b2cfb8] rounded-xl shadow-md overflow-hidden flex flex-col font-siliguri">
      ...
      <div className="bg-gradient-to-r from-[#07361f] via-[#15803d] to-[#0a4729] text-white px-3 sm:px-4 py-2.5 flex items-center justify-between border-b border-emerald-500/30 min-w-0">
        <div className="flex items-center gap-2 font-bold tracking-tight min-w-0 w-full overflow-hidden">
          ...
          <h2 className="text-xs sm:text-sm md:text-base font-bold whitespace-nowrap leading-tight">
            {language === 'bn' ? 'অ্যালামনাই কোষাধ্যক্ষ' : 'Alumni Treasurer'}
          </h2>
        </div>
      </div>
      ...
      <div className="p-3.5 flex flex-col space-y-3 bg-[#fbfdfb]">
        ...
        <div className="flex gap-3.5 items-start">
          ...
          <div className="w-24 sm:w-28 h-32 sm:h-36 rounded-lg overflow-hidden shadow-md border-2 border-[#15803d] bg-gray-100 shrink-0 relative group">
            <img
              src={TREASURER_PHOTO_URL}
              alt={language === 'bn' ? TREASURER_BIO.nameBn : TREASURER_BIO.nameEn}
              className="w-full h-full object-cover object-top"
              onError={(e) => {
                e.currentTarget.onerror = null;
                e.currentTarget.src = 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=400&q=80';
              }}
            />
          </div>
          ...
          <div className="flex-1 min-w-0 space-y-1 text-left">
            <h3 className="text-base sm:text-lg font-bold text-[#093f24] leading-tight">
              {language === 'bn' ? TREASURER_BIO.nameBn : TREASURER_BIO.nameEn}
            </h3>
            <p className="text-xs font-bold text-emerald-700 flex items-center gap-1">
              <Leaf className="w-3 h-3 text-emerald-600 shrink-0" />
              <span>{language === 'bn' ? TREASURER_BIO.titleBn : TREASURER_BIO.titleEn}</span>
            </p>
            <p className="text-[11px] font-semibold text-amber-800 bg-amber-50 border border-amber-200/60 rounded px-1.5 py-0.5 inline-flex items-center gap-1">
              ...
              <span>
                {language === 'bn'
                  ? `${TREASURER_BIO.batchBn} (সেশন: ${TREASURER_BIO.sessionBn})`
                  : `${TREASURER_BIO.batchEn} (Session: ${TREASURER_BIO.sessionEn})`}
              </span>
            </p>
          </div>
        </div>

        {/* Direct Contact details (Placed under the photo) */}
        <div className="bg-[#f0f9f3] border border-[#b2cfb8]/70 rounded-lg p-2.5 flex flex-col gap-1.5 text-xs text-gray-800 shadow-2xs">
          <a
            href={`tel:${TREASURER_BIO.phone.replace(/[^0-9]/g, '')}`}
            className="flex items-center gap-2 hover:text-[#15803d] transition-colors font-medium text-emerald-950"
          >
            <div className="p-1 rounded bg-emerald-100/90 text-emerald-700 shrink-0">
              <Phone className="w-3.5 h-3.5" />
            </div>
            <span className="font-semibold text-[13px]">{TREASURER_BIO.phone}</span>
          </a>
          <a
            href={`mailto:${TREASURER_BIO.email}`}
            className="flex items-center gap-2 hover:text-[#15803d] transition-colors font-medium text-emerald-950 min-w-0"
            title={TREASURER_BIO.email}
          >
            <div className="p-1 rounded bg-emerald-100/90 text-emerald-700 shrink-0">
              <Mail className="w-3.5 h-3.5" />
            </div>
            <span className="text-[12px] sm:text-[13px] font-medium text-gray-800 break-all">{TREASURER_BIO.email}</span>
          </a>
        </div>
        ...
        <div className="bg-[#eef8f1] border-l-3 border-[#15803d] p-2.5 rounded-md text-left">
          <p className="text-xs text-[#0a4729] font-medium leading-relaxed italic">
            "{language === 'bn' ? TREASURER_BIO.messageBn : TREASURER_BIO.messageEn}"
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 pt-0.5">
          <button
            id="btn-treasurer-speech"
            onClick={() => onOpenSpeechModal('treasurer')}
