...
export const PrimaryMembershipForm: React.FC<PrimaryMembershipFormProps> = ({
  ...
  // Auto-calculate next sequential form number starting from 0001
  useEffect(() => {
    let isMounted = true;
    const fetchNextSequence = async () => {
      try {
        ...
        snap.forEach((docSnap) => {
          ...
          const idStr = data.formNo || data.membershipId || docSnap.id || '';
        ...
        const nextNum = Math.max(totalCount + 1, maxNum + 1);
        const formattedNum = String(nextNum).padStart(4, '0');
        const generatedId = `BOT-${formattedNum}`;
        ...
        console.warn('Error fetching membership sequence:', err);
      }
    };
  ...
  const handleInputChange = (
    ...
    if (type === 'checkbox') {
      ...
      setFormData((prev) => ({
        ...
        ...(name === 'occupation' && !value ? { organizationPosition: '' } : {}),
      }));
    }
  };
  ...
  const handleFillSample = () => {
    setFormData({
      ...
      occupation: 'Teacher',
      otherOccupation: '',
      organizationPosition: 'Department of Botany, Jagannath University (Professor)',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 13. NID Number (Optional)

    // 14. Occupation & Organization / Position
    ...
    if (!formData.organizationPosition.trim()) {
      const msg = language === 'bn' 
        ? 'প্রতিষ্ঠানের নাম ও পদবী (Organization & Position) পূরণ করা বাধ্যতামূলক।' 
        : 'Organization and Position/Designation is mandatory.';
      setValidationError(msg);
      alert(msg);
      return;
    }
    ...
    try {
      const payload = {
        ...
        applicationDate: formData.applicationDate || new Date().toLocaleDateString('en-GB'),
        ...
        degreeDocName: formData.degreeDocName || degreeDocFileName || '',
        degreeDocData: formData.degreeDocData || '',
        degreeDocUrl: formData.degreeDocData || '',
        ...
        organizationPosition: formData.organizationPosition?.trim() || '',
        organization: formData.organizationPosition?.trim() || '',
        designation: formData.organizationPosition?.trim() || '',
  ...
  const handleConfirmPayment = async (payData: PaymentSubmissionData) => {
    ...
    try {
      const payload = {
        ...
        senderNumber: payData.senderNumber || 'Unpaid',
        paymentSenderNumber: payData.senderNumber || 'Unpaid',
        transactionId: payData.transactionId || 'UNPAID',
        paymentNote: payData.paymentNote || '',
        paymentStatus: payData.paymentMethod === 'Unpaid' ? 'unpaid' : 'pending',
      ...
      <div
        ...
        className={
          step === 'payment'
            ...
            : 'max-w-[210mm] w-full mx-auto bg-white border border-emerald-900/20 sm:border-2 sm:border-[#006a4e] p-3 sm:p-7 md:p-8 rounded-2xl sm:rounded-none shadow-xl text-gray-900 relative overflow-hidden print:m-0 print:p-6 print:border-2 print:border-black print:shadow-none print:min-h-[297mm] font-sans'
        }
      >
        {/* Top Header Banner */}
        <div className="w-full bg-[#006a4e] text-white p-2 sm:p-3.5 md:p-4 rounded-xl sm:rounded-t-lg border-b-4 border-amber-400 flex flex-row items-center justify-between gap-1.5 sm:gap-3 md:gap-4 relative mb-3 sm:mb-5 font-sans overflow-hidden shadow-sm">
          {/* Alumni Logo Left */}
          <div className="flex items-center shrink-0">
            <div className="w-9 h-9 min-[380px]:w-10 min-[380px]:h-10 sm:w-14 sm:h-14 md:w-16 md:h-16 flex items-center justify-center bg-white rounded-full p-0.5 sm:p-1 shadow-md border-2 border-amber-400/80">
              <img
          ...
          <div className="text-center flex-1 space-y-0.5 px-0.5 sm:px-2 min-w-0">
            <h1 className="text-[6px] min-[360px]:text-[6.5px] min-[400px]:text-[7.5px] sm:text-sm md:text-base lg:text-[17px] font-black text-amber-300 tracking-tight leading-tight uppercase font-['Plus_Jakarta_Sans',sans-serif]">
              BOTANY ALUMNI ASSOCIATION
              <span className="block text-[5px] min-[360px]:text-[5.5px] min-[400px]:text-[6.5px] sm:text-xs md:text-sm font-extrabold text-amber-200 mt-0.5">
                JAGANNATH UNIVERSITY (BAAJnU)
              </span>
            </h1>
            <p className="text-[5px] min-[360px]:text-[5.5px] min-[400px]:text-[6px] sm:text-[11px] md:text-xs font-medium text-emerald-100/90 tracking-normal font-sans leading-none">
              9-10 Chittaranjan Ave, Dhaka 1100.
            </p>
            <div className="pt-0.5">
              <span className="text-[5px] min-[360px]:text-[5.5px] min-[400px]:text-[6.5px] sm:text-xs font-bold text-white bg-[#004d38] px-1.5 min-[360px]:px-2 sm:px-3.5 py-0.5 rounded-full border border-amber-400/70 shadow-xs inline-block tracking-wide font-sans whitespace-nowrap">
          ...
          <div className="shrink-0 flex flex-col items-center font-sans">
            <label
              ...
              className={`w-11 h-13 min-[380px]:w-12 min-[380px]:h-15 sm:w-20 sm:h-24 md:w-22 md:h-26 border-2 ${
            ...
            >
              {photoPreview ? (
                <div className="relative w-full h-full">
                  ...
                  />
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[9px] sm:text-[10px] text-white font-bold transition-opacity font-sans">
                    Change
                  </div>
                </div>
              ) : (
                <div className="space-y-0.5 sm:space-y-1 text-emerald-100 px-0.5">
                  <Camera className="w-3.5 h-3.5 sm:w-5 sm:h-5 md:w-6 md:h-6 mx-auto text-amber-300 group-hover:scale-110 transition-transform" />
                  <span className="text-[6.5px] min-[380px]:text-[7.5px] sm:text-[10px] font-bold block leading-tight text-white font-sans">
                    Photo <span className="text-amber-300 font-bold">*</span>
                  </span>
                  <span className="text-[6px] min-[380px]:text-[7px] sm:text-[8.5px] text-amber-300/90 block font-sans">
                    (Upload)
                  </span>
                </div>
              )}
        ...
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 text-xs sm:text-sm font-sans">
          {/* Top Row: Date & Membership ID */}
          <div className="flex flex-col min-[480px]:flex-row justify-between items-stretch min-[480px]:items-center gap-2 mb-2 bg-emerald-50/70 sm:bg-transparent p-2.5 sm:p-0 rounded-xl sm:rounded-none border border-emerald-200/80 sm:border-none shadow-2xs sm:shadow-none">
            <div className="flex items-center gap-2 flex-1">
              ...
              <input
                ...
                className="w-full min-[480px]:w-36 border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-2.5 py-1.5 sm:py-1 text-xs sm:text-sm font-bold font-mono text-gray-900 bg-white sm:bg-emerald-50/50 focus:outline-none focus:border-[#006a4e] focus:ring-1 focus:ring-[#006a4e]"
                placeholder="DD/MM/YYYY"
              />
            </div>
            <div className="flex items-center gap-2 w-full min-[480px]:w-auto justify-between min-[480px]:justify-end pt-1.5 min-[480px]:pt-0 border-t min-[480px]:border-t-0 border-emerald-200/50">
              <span className="font-bold text-gray-900 text-xs sm:text-sm shrink-0">
                Membership ID:
              </span>
              <input
                ...
                onChange={handleInputChange}
                readOnly={step === 'success'}
                className="w-32 min-[480px]:w-40 sm:w-44 border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-2.5 py-1.5 sm:py-1 text-xs sm:text-sm font-bold font-mono text-gray-900 bg-white sm:bg-emerald-50/50 focus:outline-none focus:border-[#006a4e] focus:ring-1 focus:ring-[#006a4e]"
                placeholder="যেমন: BOT-0001"
              />
            </div>
          </div>

          {/* 1. Name of the Alumni: */}
          <div className="space-y-2 border-b border-gray-200 sm:border-gray-300 pb-2.5 sm:pb-3">
            <div className="font-bold text-[#004d38] sm:text-gray-900 text-xs sm:text-sm bg-emerald-50/90 sm:bg-transparent px-2.5 sm:px-0 py-1 sm:py-0 rounded-lg sm:rounded-none border-l-3 sm:border-l-0 border-[#006a4e]">
              1. Name of the Alumni:
            </div>
            
            <div className="grid grid-cols-1 gap-2 pl-0 sm:pl-6">
              {/* In English : */}
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                ...
                <input
                  ...
                  className="flex-1 border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-3 py-1.5 sm:py-1 text-xs sm:text-sm text-gray-900 bg-white focus:outline-none focus:border-[#006a4e] focus:ring-1 focus:ring-[#006a4e] shadow-2xs sm:shadow-none"
                />
              </div>
          ...
          <div className="space-y-2.5 border-b border-gray-200 sm:border-gray-300 pb-2.5 sm:pb-3">
            <div className="font-bold text-[#004d38] sm:text-gray-900 text-xs sm:text-sm bg-emerald-50/90 sm:bg-transparent px-2.5 sm:px-0 py-1 sm:py-0 rounded-lg sm:rounded-none border-l-3 sm:border-l-0 border-[#006a4e]">
              2. Information regarding degree earned from the Department of Botany:
            </div>

            <div className="space-y-2 pl-0 sm:pl-5">
              {/* i) BSc Honours */}
              <div className="space-y-1.5 sm:space-y-0 sm:flex sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-1.5 text-xs sm:text-sm bg-gray-50/90 sm:bg-transparent p-2.5 sm:p-0 rounded-xl sm:rounded-none border border-gray-200/80 sm:border-none">
                <span className="w-full sm:w-28 font-semibold text-gray-800 shrink-0 block sm:inline">
                  i) BSc Honours <span className="text-red-600 font-bold ml-0.5">*</span> <span className="hidden sm:inline float-right mr-1">:</span>
                </span>
                
                <div className="grid grid-cols-1 min-[480px]:grid-cols-3 sm:flex sm:flex-1 sm:items-center gap-2 sm:gap-x-3">
                  <div className="flex items-center gap-1 flex-1 min-w-[140px]">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder="1997-98"
                      className="w-full border border-gray-300 sm:border-gray-500 rounded px-1.5 py-1 text-xs text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                    />
                  </div>

                  <div className="flex items-center gap-1 flex-1 min-w-[140px]">
                    <span className="text-gray-700 text-xs shrink-0 font-medium">
                      Passing Year <span className="text-red-600 font-bold">*</span>
                    </span>
                    <input
                      type="text"
                      name="bscYear"
              ...
              <div className="space-y-1.5 sm:space-y-0 sm:flex sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-1.5 text-xs sm:text-sm bg-gray-50/90 sm:bg-transparent p-2.5 sm:p-0 rounded-xl sm:rounded-none border border-gray-200/80 sm:border-none">
                ...
                <div className="grid grid-cols-1 min-[480px]:grid-cols-3 sm:flex sm:flex-1 sm:items-center gap-2 sm:gap-x-3">
                  ...
                  <div className="flex items-center gap-1 flex-1 min-w-[140px]">
                    <span className="text-gray-700 text-xs shrink-0 font-medium">Passing Year</span>
                    <input
                      type="text"
                      name="mscYear"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-0.5">
                <div className="flex items-center gap-2 text-xs sm:text-sm bg-gray-50/70 sm:bg-transparent p-2 sm:p-0 rounded-lg sm:rounded-none border border-gray-200/60 sm:border-none">
                  ...
                  <div className="flex items-center gap-1.5 flex-1 sm:w-48">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 sm:border-gray-500 rounded px-2 py-1 text-xs text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                    />
                  </div>
                </div>
              ...
              {/* NB: Attach copy */}
              <div className="pt-1 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-gray-800 text-xs sm:text-xs bg-emerald-50/80 p-2.5 sm:p-2 rounded-xl sm:rounded border border-emerald-200">
                ...
                <div className="shrink-0 flex items-center gap-2 no-print pt-1 sm:pt-0">
                  <label
                    ...
                    className="px-3 py-1.5 sm:py-1 bg-white hover:bg-emerald-100 text-[#006a4e] border border-[#006a4e] rounded-lg sm:rounded text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-2xs transition-colors"
                  >
                  ...
                  />
                  {degreeDocFileName && (
                    <span className="text-xs font-semibold text-emerald-800 flex items-center gap-1 truncate max-w-[140px] sm:max-w-[160px]">
            ...
            <div className="space-y-2.5 pl-0 sm:pl-5">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                ...
                <input
                  ...
                  className="flex-1 border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-3 py-1.5 sm:py-1 text-xs sm:text-sm text-gray-900 bg-white focus:outline-none focus:border-[#006a4e] shadow-2xs sm:shadow-none"
                />
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3">
                ...
                <textarea
                  ...
                  className="flex-1 border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-3 py-1.5 sm:py-1 text-xs sm:text-sm text-gray-900 bg-white focus:outline-none focus:border-[#006a4e] resize-none shadow-2xs sm:shadow-none"
                />
              </div>

              {/* Permanent Address: Village, Post, Upazila, District */}
              <div className="space-y-1.5 bg-gray-50/80 sm:bg-transparent p-2.5 sm:p-0 rounded-xl sm:rounded-none border border-gray-200 sm:border-none">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 flex-1">
                    <span className="text-gray-700 text-[11px] sm:text-xs shrink-0 font-medium">
                    ...
                    <input
                      ...
                      required
                      placeholder="Village"
                      className="w-full border border-gray-300 sm:border-gray-500 rounded-md sm:rounded px-2 py-1 text-xs text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                    />
                  </div>
              ...
              {/* Date of Birth & Email */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-0.5">
                <div className="flex items-center justify-between sm:justify-start gap-2 bg-gray-50/70 sm:bg-transparent p-2 sm:p-0 rounded-lg sm:rounded-none border border-gray-200/60 sm:border-none">
                  <span className="font-semibold text-gray-800 shrink-0 text-xs sm:text-sm">
                    Date of Birth <span className="text-red-600 font-bold ml-0.5">*</span>:
                  </span>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <input
                      ...
                      required
                      placeholder="DD"
                      className="w-9 sm:w-11 text-center border border-gray-300 sm:border-gray-500 rounded px-1 py-1 text-xs font-mono font-bold text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                    />
                    ...
                    <input
                      ...
                      required
                      placeholder="YYYY"
                      className="w-13 sm:w-16 text-center border border-gray-300 sm:border-gray-500 rounded px-1 py-1 text-xs font-mono font-bold text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-2.5 py-1.5 sm:py-1 text-xs sm:text-sm text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="flex items-center gap-2">
                  <span className="w-24 sm:w-36 font-semibold text-gray-800 shrink-0 text-xs sm:text-sm">
                    Phone <span className="text-red-600 font-bold ml-0.5">*</span>:
                  </span>
                  <input
                    type="tel"
                    name="phone"
                    ...
                    className="w-full border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-2.5 py-1.5 sm:py-1 text-xs sm:text-sm font-mono font-bold text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                  />
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    required
                    className="w-full border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-2.5 py-1.5 sm:py-1 text-xs font-bold text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                  >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                <span className="w-full sm:w-36 font-semibold text-gray-800 shrink-0 text-xs sm:text-sm">
                  NID Number <span className="text-gray-400 font-normal text-xs">(Optional)</span> <span className="hidden sm:inline float-right mr-1">:</span>
                </span>
                <input
                  type="text"
                  name="nidNumber"
                  ...
                  onChange={handleInputChange}
                  placeholder="Enter ID (NID)"
                  className="flex-1 border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-3 py-1.5 sm:py-1 text-xs sm:text-sm font-mono text-gray-900 bg-white focus:outline-none focus:border-[#006a4e]"
                />
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                ...
                <div className="flex-1 flex flex-col sm:flex-row gap-1.5 sm:gap-2">
                  <select
                    ...
                    onChange={handleInputChange}
                    required
                    className={`border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-2.5 py-1.5 sm:py-1 text-xs sm:text-sm font-semibold text-gray-900 bg-white focus:outline-none focus:border-[#006a4e] ${
                  ...
                  {formData.occupation === 'Others' && (
                    <input
                      ...
                      autoFocus
                      className="flex-1 border border-[#006a4e] rounded-lg sm:rounded px-2.5 py-1.5 sm:py-1 text-xs sm:text-sm text-gray-900 focus:outline-none focus:border-[#006a4e] focus:ring-1 focus:ring-[#006a4e] bg-emerald-50/30"
                    />
                  )}
                </div>
              </div>

              {/* Organization & Position - Mandatory when Occupation is selected */}
              {formData.occupation && (
                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 transition-all duration-200 bg-emerald-50/50 sm:bg-transparent p-2 sm:p-0 rounded-lg sm:rounded-none border border-emerald-200/60 sm:border-none">
                  <span className="w-full sm:w-36 font-semibold text-gray-800 shrink-0 text-xs sm:text-sm">
                    Organization / Position <span className="text-red-600 font-bold ml-0.5">*</span> <span className="hidden sm:inline float-right mr-1">:</span>
                  </span>
                  <input
                    type="text"
                    ...
                    onChange={handleInputChange}
                    required
                    placeholder="কর্মপ্রতিষ্ঠানের নাম ও পদবী (যেমন: জগন্নাথ বিশ্ববিদ্যালয় - অধ্যাপক) *"
                    className="flex-1 border border-gray-300 sm:border-gray-500 rounded-lg sm:rounded px-3 py-1.5 sm:py-1 text-xs sm:text-sm text-gray-900 bg-white focus:outline-none focus:border-[#006a4e] focus:ring-1 focus:ring-[#006a4e] shadow-2xs sm:shadow-none"
                  />
                </div>
              )}
          ...
          {/* Declaration Line */}
          <div className="pt-1 pb-1 bg-emerald-50/60 sm:bg-transparent p-2.5 sm:p-0 rounded-xl sm:rounded-none border border-emerald-200/80 sm:border-none">
            <label className="flex items-start sm:items-center gap-2 cursor-pointer select-none">
              <input
                ...
                onChange={handleInputChange}
                className="w-4 h-4 text-[#006a4e] rounded border-gray-400 focus:ring-emerald-500 cursor-pointer mt-0.5 sm:mt-0 shrink-0"
              />
          ...
          {validationError && (
            <div className="no-print p-2.5 sm:p-3 bg-red-50 border-l-4 border-red-600 rounded-lg text-red-800 text-xs sm:text-sm font-semibold flex items-center gap-2 shadow-2xs">
          ...
          {paymentInfo && (
            <div className="mt-2 pt-2 border-t border-dashed border-gray-400 font-sans">
              <div className={`border rounded-lg p-2 sm:p-2.5 flex flex-wrap items-center justify-between gap-1.5 text-xs ${
                paymentInfo.paymentMethod === 'Unpaid' || paymentInfo.transactionId === 'UNPAID' || (paymentInfo as any).paymentStatus === 'unpaid'
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {paymentInfo.paymentMethod === 'Unpaid' || paymentInfo.transactionId === 'UNPAID' || (paymentInfo as any).paymentStatus === 'unpaid' ? (
                    <span className="font-extrabold text-red-800 bg-red-100 px-2 py-0.5 rounded border border-red-300 font-mono text-xs uppercase">
                      Unpaid (৳ {paymentInfo.feeAmount}/-)
                    </span>
                  ) : (
                    <span className="font-bold text-emerald-800 bg-emerald-100 px-1.5 py-0.5 rounded border border-emerald-300 font-mono text-xs">
                      PAID (৳ {paymentInfo.feeAmount}/-)
                    </span>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-2 font-mono text-xs text-gray-700">
                  <span>Method: <strong>{paymentInfo.paymentMethod}</strong></span>
                  {paymentInfo.paymentMethod !== 'Unpaid' && (
                    <>
          ...
          {/* Action Buttons */}
          {step === 'form' && (
            <div className="no-print pt-3 sm:pt-5 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-center gap-2.5 sm:gap-3">
              <button
                type="submit"
              ...
              >
                ...
                <span className="text-amber-300 font-extrabold text-xs sm:text-sm ml-1">→</span>
              </button>

              <button
                type="button"
                onClick={handleResetForm}
                className="w-full sm:w-auto px-4 py-2 sm:py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold rounded-xl transition-colors flex items-center justify-center gap-1.5 cursor-pointer border border-gray-300 order-2 sm:order-1"
              >
