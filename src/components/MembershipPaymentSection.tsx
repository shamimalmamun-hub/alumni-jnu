...
export interface PaymentSubmissionData {
  paymentMethod: 'bKash' | 'Nagad' | 'Rocket' | 'Bank Transfer' | 'Unpaid';
  senderNumber: string;
...
export const MembershipPaymentSection: React.FC<MembershipPaymentSectionProps> = ({
  ...
  const [membershipType, setMembershipType] = useState<'general' | 'life' | 'unpaid'>(
    formData.membershipType || 'general'
  );
  const [paymentMethod, setPaymentMethod] = useState<'bKash' | 'Nagad' | 'Rocket' | 'Bank Transfer' | 'Unpaid'>('bKash');
  ...
  const feeAmountBn = membershipType === 'life' ? '২৫০০' : membershipType === 'unpaid' ? 'Unpaid' : '৫০০';
  const feeAmountEn = membershipType === 'life' ? '2500' : membershipType === 'unpaid' ? 'Unpaid' : '500';
  const feeInWords =
    membershipType === 'life'
      ? (language === 'bn' ? '(দুই হাজার পাঁচশত টাকা মাত্র)' : '(Two Thousand Five Hundred BDT)')
      : membershipType === 'unpaid'
      ? (language === 'bn' ? '(পরে ফি পরিশোধ বা ক্যাশ জমা)' : '(Fee to be paid later or in cash)')
      : (language === 'bn' ? '(পাঁচশত টাকা মাত্র)' : '(Five Hundred BDT)');
  const typeNameBn = membershipType === 'life' ? 'আজীবন সদস্য' : membershipType === 'unpaid' ? 'Unpaid Option (পরে ফি জমা)' : 'সাধারণ সদস্য';
  const typeNameEn = membershipType === 'life' ? 'Life Member' : membershipType === 'unpaid' ? 'Unpaid Option' : 'General Member';
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    const isUnpaidOpt = paymentMethod === 'Unpaid';
    const finalSenderNumber = isUnpaidOpt ? (senderNumber.trim() || 'Unpaid') : senderNumber.trim();
    const finalTransactionId = isUnpaidOpt ? (transactionId.trim() || 'UNPAID') : transactionId.trim();

    // Validation for paid methods
    if (!isUnpaidOpt && !senderNumber.trim()) {
      const msg =
        language === 'bn'
    ...
    if (!isUnpaidOpt && !transactionId.trim()) {
      const msg =
        language === 'bn'
  ...
  return (
    <div className="w-full max-w-3xl mx-auto py-4 sm:py-6 px-3 sm:px-4 font-sans text-gray-900 animate-in fade-in duration-300">
      ...
      {/* Dedicated Membership Category Selection Section */}
      <div className="bg-emerald-50/90 border-2 border-emerald-600 rounded-xl p-4 sm:p-5 mb-6 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
          ...
          >
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="membership-general-opt" className="cursor-pointer font-black text-sm text-gray-900">
                  {language === 'bn' ? 'সাধারণ সদস্য' : 'General Member'}
                </label>
              </div>
              <span className="inline-block px-2.5 py-0.5 rounded-lg text-xs font-black font-mono bg-emerald-100 text-[#006a4e] border border-emerald-300 shadow-2xs">
                ৳ ৫০০/-
              </span>
            </div>
            <p className="text-[11px] text-gray-500 font-medium leading-snug">
              {language === 'bn' ? 'সাধারণ ২ বছর মেয়াদী সদস্য নিবন্ধন' : '2-Year General Membership'}
            </p>
          </div>
          ...
          >
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="membership-life-opt" className="cursor-pointer font-black text-sm text-gray-900">
                  {language === 'bn' ? 'আজীবন সদস্য' : 'Life Member'}
                </label>
              </div>
              <span className="inline-block px-2.5 py-0.5 rounded-lg text-xs font-black font-mono bg-amber-100 text-amber-800 border border-amber-300 shadow-2xs">
                ৳ ২৫০০/-
              </span>
            </div>
            <p className="text-[11px] text-gray-500 font-medium leading-snug">
              {language === 'bn' ? 'আজীবন স্থায়ী সদস্যপদ ও ভোটার অধিকার' : 'Permanent Life Membership'}
            </p>
          </div>
          ...
          <div
            ...
            className={`p-3.5 rounded-xl border-2 cursor-pointer transition-all flex flex-col justify-between ${
              membershipType === 'unpaid'
                ? 'bg-red-50/90 border-red-600 ring-2 ring-red-600/20 shadow-sm'
                : 'bg-white/70 border-gray-300 hover:border-red-400 hover:bg-white'
            }`}
          >
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                <input
                  type="radio"
                  id="membership-unpaid-opt"
                  name="membershipCategory"
                  value="unpaid"
                  checked={membershipType === 'unpaid'}
                  onChange={() => {
                    setMembershipType('unpaid');
                    setPaymentMethod('Unpaid');
                    setSenderNumber('Unpaid');
                ...
                />
                <label htmlFor="membership-unpaid-opt" className="cursor-pointer font-black text-sm text-gray-900">
                  {language === 'bn' ? 'Unpaid Option' : 'Unpaid Option'}
                </label>
              </div>
              <span className="inline-block px-2.5 py-0.5 rounded-lg text-xs font-black font-mono bg-red-800 text-white border border-red-900 shadow-2xs">
                Unpaid
              </span>
            </div>
            <p className="text-[11px] text-gray-600 font-medium leading-snug">
              {language === 'bn' ? 'পরে ফি পরিশোধ বা ম্যানুয়ালি ক্যাশ জমা' : 'Pay membership fee later or in cash'}
            </p>
          </div>
        </div>
      ...
      <form onSubmit={handlePaymentSubmit} className="bg-white border-2 border-emerald-700/60 rounded-xl p-4 sm:p-6 shadow-lg space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 sm:gap-2">
            {[
              ...
              <button
                ...
                className={`p-2.5 rounded-xl border-2 transition-all cursor-pointer flex flex-col items-center justify-between text-center relative min-h-[96px] ${
                  paymentMethod === opt.id
                    ? opt.id === 'Unpaid'
                      ? 'bg-amber-50/90 border-amber-600 ring-2 ring-amber-500/20 shadow-sm'
              ...
              >
                ...
                {/* Logo Image or Bank / Unpaid Icon */}
                <div className="h-10 w-full flex items-center justify-center px-1 my-0.5">
                  {'logo' in opt && opt.logo ? (
                    <img
                      ...
                      referrerPolicy="no-referrer"
                    />
                  ) : 'isUnpaid' in opt && opt.isUnpaid ? (
                    <div className="px-2 py-1 rounded-lg bg-red-100 text-red-800 border border-red-300 flex items-center justify-center shadow-2xs font-mono font-black text-xs uppercase">
                      Unpaid
                    </div>
                  ) : (
        ...
        {paymentMethod === 'Unpaid' ? (
          <div className="p-4 bg-amber-50 border-2 border-amber-300 rounded-xl text-amber-950 text-xs font-semibold flex items-center gap-3 shadow-2xs">
            <AlertCircle className="w-6 h-6 shrink-0 text-amber-600" />
            <div className="space-y-0.5">
              <p className="font-extrabold text-sm text-amber-900">Unpaid (পরে পরিশোধ) অপশন নির্বাচিত হয়েছে</p>
              <p className="text-xs text-amber-800">
                {language === 'bn'
                  ? 'কোনো পেমেন্ট ছাড়াই এখন ফর্ম সাবমিট করতে পারবেন। আবেদনপত্র প্রিন্ট করলে নিচে "Unpaid" প্রদর্শিত হবে এবং পরবর্তীতে ফি জমা দিয়ে পরিশোধ করতে পারবেন।'
                  : 'You can submit the form without upfront payment. The printed form will show "Unpaid" at the bottom.'}
              </p>
            </div>
          </div>
        ) : (
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-700 mb-1">
            {paymentMethod === 'Unpaid' ? '২. মন্তব্য বা দ্রষ্টব্য (ঐচ্ছিক / Optional)' : '৪. অতিরিক্ত নোট বা মন্তব্য (ঐচ্ছিক / Optional)'}
          </label>
          <input
            type="text"
            value={paymentNote}
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className={`w-full sm:w-auto px-6 py-3 disabled:bg-gray-400 text-white text-xs sm:text-sm font-bold rounded-lg shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-98 ${
              paymentMethod === 'Unpaid'
                ? 'bg-amber-600 hover:bg-amber-700'
                : 'bg-[#006a4e] hover:bg-[#00523d]'
            }`}
          >
            {isSubmitting ? (
              <>
              ...
              <>
                ...
                <span>
                  {language === 'bn'
                    ? 'Unpaid হিসেবে ফরম সাবমিট করুন'
                    : 'Submit Form as Unpaid'}
                </span>
              </>
            ) : (
              <>
